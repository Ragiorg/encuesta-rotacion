...
function calculateHerzbergByDepartment(responses: any[]) {
  const departments = Array.from(new Set(responses.map(r => r.department || 'No especificado')))
...
function generateRecommendations(herzbergAnalysis: any, advancedMetrics: any, responses: any[]) {
  const recommendations: { type: string; priority: string; title: string; description: string; actions: string[] | string[]; expectedImpact: string; timeframe: string }[] = []
